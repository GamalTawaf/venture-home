...
class VentureFactory(factory.django.DjangoModelFactory):
    ...
    stage = factory.LazyAttribute(
        lambda x: fake.random_element(elements=("Ideation", "Pilot", "Growth", "Scale"))
    )
    founder = factory.LazyAttribute(lambda x: fake.name())
    metrics = factory.LazyAttribute(
        lambda x: {
    ...
    status = factory.LazyAttribute(
        lambda x: fake.random_element(elements=("on_track", "at_risk", "off_track"))
    )
